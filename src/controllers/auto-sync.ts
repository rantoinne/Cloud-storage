...
const mapFileSyncToFileUpload = ({ path, type, size }: FileSync): FileUploadLiteType => {
  return { path, type, size, name: path.split('/').pop(), isAutoSync: true, destDir: '/' }
}

export class AutoSyncController {
  static intialized = false

  static async setConfig(config?: FileScannerConfig) {
    if (isIOS || disableSync) return false
    const res = await RNFileScanner.setConfig(config)
    if (res) AutoSyncController.intialized = true
    return res
  }

  static onScanStatus = RNFileScanner.onScanStatus

  static onFileChange = RNFileScanner.onFileChange
  ...
  */
  static async getNextToSyncPage(
    lastGlobalIndex: number,
  ): Promise<{ list: FileUploadLiteType[]; lastGlobalIndex: number }> {
    ...
    while (list.length < MAX_PAGE_SIZE) {
      ...
      list = list.concat(
        data
          ...
          // Convert FileSync to FileUpload object model
          .map(mapFileSyncToFileUpload),
      )
      page++
    }
