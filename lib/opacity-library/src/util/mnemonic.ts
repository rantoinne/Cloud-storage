 ...
 */
export const ACCOUNT_DERIVE_PATH =
  "m/43'/60'/1775'/0'/" + hashToPath(nameHash('opacity.io'));

export const createMnemonic = async (): Promise<string[]> => {
  return generateMnemonic().split(' ');
};

export const mnemonicToHandle = async (
  mnemonic: string[],
): Promise<Uint8Array> => {
  ...
  return new Uint8Array(
    Array.from(hd.privateKey).concat(Array.from(hd.chainCode)),
  );
};
...
export const retrieveHandle = async (
  mnemonic: string[],
): Promise<Uint8Array> => {
  try {
    if (mnemonic.length !== 12) {
      throw new Error('Totally 12 words need!');
    }
